...
@torch.no_grad()
def _sur_score(gp, ref_X: torch.Tensor, cand: torch.Tensor) -> float:
    """SUR score of a single candidate (1, D) over reference design ``ref_X`` (probit)."""
    acqf = ContourSUR(gp, ref_X=ref_X, link="probit")
    return float(acqf(cand.unsqueeze(-2)).item())


@torch.no_grad()
def _sur_pointwise(gp, ref_X: torch.Tensor, cand: torch.Tensor) -> np.ndarray:
    """Per-reference-point error reduction ``w_u = Phi(-|mu_u|/s_u) - Phi(-|mu_u|/s_{n+1,u})``
    (≥ 0) that the chosen candidate's fantasized observation induces — the per-point
    contribution to SUR's mean. Mirrors :meth:`ContourSUR.forward` but returns the vector."""
    M = ref_X.shape[0]
    ...
    return (now - fut).clamp_min(0.0).numpy()
...
@torch.no_grad()
def _fidelity(gp, op_ref: torch.Tensor, pool: torch.Tensor) -> tuple[float, int, float]:
    ...
    link; SUR uses the run's operating reference design."""
    s_sur = ContourSUR(gp, ref_X=op_ref, link="probit")(pool.unsqueeze(-2)).numpy()
    s_gsur = ContourGSUR(gp, link="probit")(pool.unsqueeze(-2)).numpy()
...
def generate_sur_refsize(config_dir: Path) -> Path:
    ...
    sensitivity views; cache to ``analysis/sur_refsize/data.npz``. Returns the cache dir."""
    ...
    D = Xt.shape[1]
    cfg = _gp_cfg(algo, D, target.dim(), max_rank,
                  torch.tensor(target.shape, dtype=torch.double))
    ...
    for i, st in enumerate(a_steps):
        ...
        for j, m in enumerate(m_grid):
            scores = [_sur_score(gp, _ref_design(D, m, 1000 * m + d), cand) for d in range(_K_A)]
            a_mean[i, j], a_std[i, j] = float(np.mean(scores)), float(np.std(scores))

    # The run's actual operating reference design (first op_M of the seeded n_ref Sobol
    # cover — exactly what the run used) and a fixed candidate pool for the fidelity view.
    ...
    for i, st in enumerate(tl_steps):
        gp, cand = gp_and_cand(int(st))
        scores = [_sur_score(gp, _ref_design(D, op_M, 7000 + d), cand) for d in range(_K_B)]
        ...
        fid_spearman[i], fid_top1[i], fid_top10[i] = _fidelity(gp, op_ref, pool)

    out = _refsize_dir(config_dir)
    out.mkdir(parents=True, exist_ok=True)
    np.savez(out / "data.npz",
             op_M=op_M, n_ref=n_ref, pool_size=int(pool.shape[0]),
             a_steps=a_steps, a_M=np.array(m_grid), a_mean=a_mean, a_std=a_std,
             tl_steps=tl_steps, tl_cv=tl_cv, tl_peff=tl_peff,
