"""means.py — GP prior-mean functions for the BO-family surrogates, in one place.

:func:`make_mean` is the single factory both surrogates call (BOSS's regression GP
and cBOSS's feasibility classifier). Available means:
...
def make_mean(name: str, D: int, *, N: int | None = None,
    ...
    """Build a GP prior mean by name. ``log_size`` additionally needs ``N``,
    ``max_rank`` and ``t_shape`` (the physical mode sizes); the gpytorch means
    need only ``D``."""
    if name == "constant":
        return ConstantMean()
    if name == "linear":
