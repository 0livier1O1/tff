...
class BOSS(object):
    def __init__(
        ...
        self.D = int(
            (N * (N - 1)) / 2
        )  # Number of parameter`s (i.e. number of off-diagonal elements of adjacency matrix)
        ...
        self.bounds[
            1
        ] *= max_rank  # Max rank for each node  # TODO is it better for each node's rank to be its own max?
        ...
        self.model_cr = CompressionRatio(
            target=self.target, bounds=self.bounds, diag=self.t_shape
        )
        self.decomp = decomp
    ...
    def __call__(self, X_init=None, callback=None):
        ...
        self.train_X, self.train_Y, self.train_t = self._initial_points(
            bounds=std_bounds, X_init=X_init
        )
        ...
        for b in range(self.budget):
            ...
            cr_curr, rse_curr = (
                self.train_Y[-1][0].item(),
                self.train_Y[-1][-1].exp().item(),
            )
            ...
            if self.verbose:
                print(
                    f"Iter {b}: Selected CR={cr_curr:0.5f} --- RSE={rse_curr:0.5f} --- AF={af.item():0.4f}"
                )
    ...
    def get_model(self, X=None, y=None):
        if X is None or y is None:
            X, y = self.train_X.to(dtype=torch.double), self.train_Y.to(
                dtype=torch.double
            )
    ...
    def _optimize_acqf(self, acqf, bounds):
        with warnings.catch_warnings(), gpsttngs.fast_pred_samples(
            state=True
        ), gpsttngs.fast_computations(
    ...
    def _get_obj_and_constraint(self, raw_x: Tensor):
        x = (
            unnormalize(raw_x, bounds=self.bounds).round().to(torch.int)
        )  # raw_x is a vector in unit cube -> turn into integer ranks
        A = triu_to_adj_matrix(x, diag=self.t_shape).squeeze(
            1
        ...
        if self.n_workers == 1 or raw_x.shape[0] == 1:
            ...
            runtime = []
            loop = (
                tqdm(A.unbind(), desc="TN eval")
                if self.verbose and raw_x.shape[0] > 1
                else A.unbind()
            )
            for a in loop:
            ...
            objectives = progress_starmap(
                self._evaluate_tn, par_args, n_cpu=self.n_workers, total=len(par_args)
            )
        runtime = torch.tensor(runtime)
        objectives = torch.concat(
            [torch.tensor(cr).unsqueeze(1), torch.stack(rse)], axis=1
        )
    ...
    def _evaluate_tn(self, A):
        ...
        while i < self.tn_runs:
            ...
            if self.decomp == "FCTN":
                ...
                rse = decomp_pam(
                    self.target, A.to(torch.int), tol=None, iter=self.maxiter_tn
                )
            else:
                raise NotImplementedError
                rse, n_epochs = t_ntwrk.decompose(
                    self.target,
                    tol=None,
